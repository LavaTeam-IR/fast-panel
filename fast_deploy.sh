...
EOF
    ...
    PROJECT_REQ=$(curl -s -X POST https://backboard.railway.app/graphql/v2 \
    ...
    DEPLOY_GQL="{\"query\": \"mutation { serviceCreate(input: { projectId: \\\"$PROJECT_ID\\\", source: { repo: \\\"LavaTeam-IR/fast-panel\\\" } }) { id } }\"}"
    SERVICE_REQ=$(curl -s -X POST https://backboard.railway.app/graphql/v2 \
      -H "Authorization: Bearer $RAILWAY_TOKEN" \
      -H "Content-Type: application/json" \
      -d "$DEPLOY_GQL")
