...
app = Flask(__name__)
# NOTE: If you don't set a SECRET_KEY env var, a random one is generated at
# startup. That's more secure than a hardcoded default, but it means every
# restart invalidates existing sessions (fine for a single-admin panel).
...
def load_data():
    ...
    try:
        ...
        data = {
            "config": {"password": generate_password_hash("admin"), "clean_ips": []},
            "inbounds": {},
            "traffic": {"total_usage": 0, "history": []}
        }

    # Migrate any legacy plaintext password (e.g. old "admin" default) to a hash.
    pw = data.get("config", {}).get("password", "")
    if pw and not pw.startswith(("pbkdf2:", "scrypt:")):
        data["config"]["password"] = generate_password_hash(pw)
        save_data(data)
...
# IMPORTANT FIX: DASHBOARD_CONTENT / INBOUNDS_CONTENT / CLEAN_IP_CONTENT /
# SECURITY_CONTENT all do `{% extends "base" %}`, but Flask's default loader
# only looks for template FILES on disk (in a templates/ folder) — it has no
# idea "base" refers to the BASE_TEMPLATE string above. Without this, every
# page except /login raised jinja2.exceptions.TemplateNotFound: base and
# crashed with a 500 error. A DictLoader maps the name "base" to the actual
# string so `extends` can find it, while still falling back to the normal
# file-based loader for anything else.
import jinja2
app.jinja_loader = jinja2.ChoiceLoader([
    jinja2.DictLoader({"base": BASE_TEMPLATE}),
    app.jinja_loader,
])
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                ...
                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td class="p-4 text-left space-x-1 space-x-reverse">
                        ...
                        <form action="{{ url_for('toggle_inbound', id=id) }}" method="POST" class="inline-block">
                            <button type="submit" class="bg-amber-50 hover:bg-amber-100 text-amber-600 dark:bg-amber-950/40 dark:text-amber-400 p-2 rounded-lg text-sm" title="تغییر وضعیت"><i class="fas fa-power-off"></i></button>
                        </form>
                        <form action="{{ url_for('delete_inbound', id=id) }}" method="POST" class="inline-block" onsubmit="return confirm('آیا از حذف این کاربر مطمئن هستید؟')">
                            <button type="submit" class="bg-rose-50 hover:bg-rose-100 text-rose-600 dark:bg-rose-950/40 dark:text-rose-400 p-2 rounded-lg text-sm" title="حذف"><i class="fas fa-trash-alt"></i></button>
                        </form>
                    </td>
                </tr>
                {% endfor %}
...
<div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
    ...
    <div class="divide-y divide-slate-100 dark:divide-slate-700/50">
        ...
        <div class="p-4 flex justify-between items-center hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-all">
            ...
            <form action="{{ url_for('delete_clean_ip', ip=ip) }}" method="POST" onsubmit="return confirm('آیا از حذف این آی‌پی مطمئن هستید؟')">
                <button type="submit" class="bg-rose-50 hover:bg-rose-100 text-rose-600 dark:bg-rose-950/40 dark:text-rose-400 p-2 rounded-lg text-sm"><i class="fas fa-trash-alt"></i></button>
            </form>
        </div>
        {% endfor %}
    </div>
...
# Very small in-memory brute-force guard. Resets on restart, and is per-process
# (fine for a single free-tier instance, not a substitute for a real WAF).
_login_attempts = {}
MAX_ATTEMPTS = 5
LOCKOUT_SECONDS = 60
...
def login():
    ...
    if request.method == "POST":
        ...
        if time.time() < locked_until:
            ...
            return render_template_string(LOGIN_TEMPLATE, error=f"تعداد تلاش‌ها بیش از حد مجاز بود. {wait} ثانیه دیگر دوباره امتحان کنید.")

        data = load_data()
        if check_password_hash(data["config"]["password"], request.form.get("password", "")):
        ...
        if attempts >= MAX_ATTEMPTS:
            _login_attempts[ip] = (0, time.time() + LOCKOUT_SECONDS)
            return render_template_string(LOGIN_TEMPLATE, error=f"تعداد تلاش‌های ناموفق بیش از حد بود. {LOCKOUT_SECONDS} ثانیه صبر کنید.")
...
@app.route("/inbounds/toggle/<id>", methods=["POST"])
...
@app.route("/inbounds/delete/<id>", methods=["POST"])
...
@app.route("/clean-ips/delete/<ip>", methods=["POST"])
...
def security():
    ...
    if request.method == "POST":
        ...
        if new_password:
            data = load_data()
            data["config"]["password"] = generate_password_hash(new_password)
            save_data(data)
            success = True
...
@app.route("/fast-ws")
def ws_tunnel():
    # IMPORTANT: this is a placeholder only. It does not upgrade the
    # connection to a WebSocket and does not proxy/relay any VLESS traffic.
    # A real implementation needs an actual WS server (e.g. flask-sock,
    # websockets, or a reverse proxy in front of Flask) that terminates the
    # WebSocket and forwards bytes to a VLESS inbound. Clients pointed at
    # this URL today will NOT get a working VPN connection.
